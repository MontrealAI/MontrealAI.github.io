...
const APP = {
  ...
  storageKey: 'goalos_uvsi3_core_v8_0_0',
...
function environmentSpec() {
  return {schema:'GoalOS.EnvironmentSpec.v8',projectId:state.meta.projectId,missionId:safeName(state.mission.name),version:state.gym.version,mission:deepClone(state.mission),state:state.gym.spec.state,observations:state.gym.spec.observations,actions:state.gym.spec.actions,rewards:state.gym.spec.rewards,constraints:state.gym.spec.constraints,scenarioDistribution:state.gym.distribution,seeds:{formation:state.gym.formationSeed,proof:state.gym.proofSeed,transfer:state.gym.transferSeed},generatedAt:nowISO()};
}
function candidateManifest() {
  const candidate=candidateById(state.proof.frozenCandidateId);
  return candidate?{schema:'GoalOS.CandidateSuccessorManifest.v8',projectId:state.meta.projectId,candidate:deepClone(candidate),manifestHash:state.proof.manifestHash,frozenAt:state.proof.frozenAt,missionHash:hashObject(state.mission),environmentVersion:state.gym.version,externalAuthorityCreated:'NONE'}:{schema:'GoalOS.CandidateSuccessorManifest.v8',status:'NO_FROZEN_CANDIDATE'};
}
...
function missionPackFiles() {
  ...
  return [
    ...
    {name:`${name}_Authority_Envelope.json`,data:JSON.stringify({schema:'GoalOS.AuthorityEnvelope.v8',...state.authority},null,2)},
    {name:`${name}_Chronicle.json`,data:JSON.stringify({schema:'GoalOS.Chronicle.v8',records:state.chronicle,negativeCapability:state.negativeCapability,successorBook:state.successorBook,requalification:state.requalification},null,2)},
    {name:`${name}_Recursive_Improvement.json`,data:JSON.stringify(recursiveReport(),null,2)},
    {name:`${name}_Bounded_AGI_Jobs.json`,data:JSON.stringify({schema:'GoalOS.BoundedAGIJobPortfolio.v8',jobs:state.jobs},null,2)},
    {name:`${name}_Formation_Results.csv`,data:formationCSV()}
  ];
}
function exportMissionPack() { const name=safeName(state.mission.name); downloadBlob(`${name}_GoalOS_Mission_Pack_v8.zip`,makeZip(missionPackFiles())); record('MISSION_PACK_EXPORTED','Complete Mission Pack exported',name); toast('Mission Pack ZIP downloaded'); }
