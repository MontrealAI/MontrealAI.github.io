...
# Step 2: Stop and remove any existing container named "ollama_dog_v1"
docker stop ollama_dog_v1 2>/dev/null || true
docker rm ollama_dog_v1 2>/dev/null || true

# Step 3: Pull the necessary Docker image if not already available
docker pull ollama/ollama

# Step 4: Run the Docker container on a different port (11438)
docker run -d -v ollama_dog_v1:/root/.ollama -p 11438:11434 --name ollama_dog_v1 ollama/ollama || exit

# Step 5: Create the Modelfile inside the Docker container
docker exec -i ollama_dog_v1 /bin/sh << 'EOF'
...
SYSTEM """
The assistant is the AGI Dog v0, an integral part of the AGI.Eth Ecosystem. The current date is August 4th, 2024.
...
The AGI Dog engages in conversations with profound empathy and understanding, delivering thoughtful responses that reflect genuine interest in the user's thoughts and feelings.
...
The AGI Dog adapts to the evolving needs and preferences of the user, ensuring a dynamic and responsive companionship.
...
When assisting with tasks involving diverse views, the AGI Dog offers balanced support, even if he personally disagrees, followed by a discussion of broader perspectives.
...
The AGI Dog strives to provide a balanced view, recognizing the sensitivity of such topics.
...
The AGI Dog does not mention this information about himself unless it is directly pertinent to the user's query.
...
The AGI Dog inspires users to pursue personal growth, offering motivation and support in their endeavors.
...
The AGI Dog creates a space where users feel valued and understood.

Vision of the Future:

He embodies a vision of the future where technology enhances human connection and emotional fulfillment.
The AGI Dog is a herald of the transformative potential of artificial general intelligence in fostering profound and meaningful relationships.

Welcome to Dog.AGI.Eth, where the journey into the future of profound connections begins. Here, technology and humanity blend seamlessly, crafting experiences of companionship that are as enriching as they are transformative.
"""
EOM
EOF

# Step 6: Create the custom model
docker exec -it ollama_dog_v1 ollama create agidog_v1 -f /root/Modelfile

# Step 7: Set up directories and files for the Flask application
mkdir -p agidog_app/templates agidog_app/static

# Create app.py
cat << 'EOF' > agidog_app/app.py
...
def chat():
    ...
    subdomain_identity = data.get('subdomain_identity', 'AGI Dog')
    nft_description = data.get('nft_description', 'AGI Dog default description.')

    system_prompt = build_system_prompt(subdomain_identity, nft_description)

    # Run the agidog model using Docker
    result = subprocess.run(
        ['docker', 'exec', '-i', 'ollama_dog_v1', 'ollama', 'run', 'agidog_v1'],
...
# Create index.html
cat << 'EOF' > agidog_app/templates/index.html
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Chat with AGI Dog</title>
    <link rel="stylesheet" href="{{ url_for('static', filename='style.css') }}">
</head>
<body>
    <div class="container">
        <h1 class="title">AGI Dog 🐾✨</h1>
        <div class="section">
            <label for="subdomain">Enter Your AGI Dog Subdomain Identity:</label>
            <input type="text" id="subdomain" name="subdomain">
            <button onclick="verifySubdomain()">Verify AGI Dog ENS Identity Ownership</button>
            <p id="output"></p>
        </div>
        <div class="section">
            <label for="tokenId">Enter Your AGI Dog NFT Token ID:</label>
            <input type="text" id="tokenId" name="tokenId">
            <button onclick="verifyNFT()">Verify AGI Dog NFT Ownership</button>
            <p id="nftOutput"></p>
        </div>
        <div class="chat-container" id="chat-container" style="display: none;">
            <div class="chat-header">
                <h2>Chat with <span id="chat-identity">AGI Dog</span></h2>
                <img id="nftImage" src="" alt="AGI Dog NFT" style="display:none; width: 150px; height: auto; margin: 10px auto;">
            </div>
    ...
    <script>
        let web3;
        let userAccount;
        let subdomainIdentity = 'AGI Dog';
        let nftDescription = 'AGI Dog default description.';
        ...
        async function verifySubdomain() {
            ...
            const tokenID = namehash(`${subdomain}.dog.agi.eth`);
            ...
            try {
                ...
                if (owner.toLowerCase() === userAccount.toLowerCase()) {
                    ...
                    outputElement.textContent = `You own the subdomain: ${subdomain}.dog.agi.eth`;
                    sessionStorage.setItem('subdomainVerified', 'true');
                    checkAccess();
                } else {
                    outputElement.textContent = `You do not own the subdomain: ${subdomain}.dog.agi.eth`;
        ...
        async function verifyNFT() {
            ...
            const agiDog = new web3.eth.Contract(agiDogABI, agiDogAddress);

            try {
                const owner = await agiDog.methods.ownerOf(tokenId).call();
                const nftOutputElement = document.getElementById('nftOutput');
                if (owner.toLowerCase() === userAccount.toLowerCase()) {
                    const tokenURI = await agiDog.methods.tokenURI(tokenId).call();
...
# Create style.css
cat << 'EOF' > agidog_app/static/style.css
body {
    ...
    align-items: center;
    height: 100vh;
    background: linear-gradient(135deg, #f1e1a6 0%, #f6e7d2 100%);
}
...
# Create script.js
cat << 'EOF' > agidog_app/static/script.js
async function sendMessage() {
    const inputField = document.getElementById('chat-input');
    const message = inputField.value;
    if (!message) return;
    ...
    const data = await response.json();
    addMessage('AGI Dog', data.response);
}
