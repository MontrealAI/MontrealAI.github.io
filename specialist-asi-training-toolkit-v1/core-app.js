...
const APP = {
  ...
  version: '9.0.0-UVSI3',
  edition: 'Sovereign Specialist ASI Training Toolkit Ω · UVSI3',
  storageKey: 'goalos_uvsi3_core_v9_0_0',
...
function missionPackFiles() {
  ...
  return [
    ...
    {name:`${name}_Authority_Envelope.json`,data:JSON.stringify({schema:'GoalOS.AuthorityEnvelope.v9',...state.authority},null,2)},
    {name:`${name}_Chronicle.json`,data:JSON.stringify({schema:'GoalOS.Chronicle.v9',records:state.chronicle,negativeCapability:state.negativeCapability,successorBook:state.successorBook,requalification:state.requalification},null,2)},
    {name:`${name}_Recursive_Improvement.json`,data:JSON.stringify(recursiveReport(),null,2)},
    {name:`${name}_Bounded_AGI_Jobs.json`,data:JSON.stringify({schema:'GoalOS.BoundedAGIJobPortfolio.v9',jobs:state.jobs},null,2)},
    {name:`${name}_Formation_Results.csv`,data:formationCSV()}
  ];
}
function exportMissionPack() { const name=safeName(state.mission.name); downloadBlob(`${name}_GoalOS_Mission_Pack_v9_0_0.zip`,makeZip(missionPackFiles())); record('MISSION_PACK_EXPORTED','Complete Mission Pack exported',name); toast('Mission Pack ZIP downloaded'); }
...
function init() {
  ...
  if(!state.chronicle.length)record('PROJECT_CREATED','GoalOS UVSI3 project created',`${state.mission.name} · ${APP.edition}.`);
  renderAll();
}
