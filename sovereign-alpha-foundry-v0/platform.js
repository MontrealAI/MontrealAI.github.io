...
const ACCESS_KEY = 'goalos_uvsi3_access_v8_0_0';
...
async function runAI(action){
  ...
  try{
    ...
    if(platform.ai.endpoint){
      const response=await fetch(platform.ai.endpoint.replace(/\/$/,'')+'/api/analyze',{method:'POST',headers:{'Content-Type':'application/json','X-GoalOS-App':'goalos-uvsi3-v8'},body:JSON.stringify({action,language:lang(),project:compactCore(),evidence:platform.ai.consent?platform.evidence:[],accessReceipt:getReceipt(),schema:aiSchema()})});
