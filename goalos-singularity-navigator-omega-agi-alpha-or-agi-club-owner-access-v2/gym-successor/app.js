...
const APP = {
  name: 'GoalOS Gym × Successor',
  version: '5.0.0-UVSI1',
  edition: 'Unified Verified Succession Institution compatibility layer',
  storageKey: 'goalos_gym_successor_recursive_v4_0_0',
  legacyStorageKeys: ['goalos_gym_successor_recursive_v3_0_0','goalos_gym_successor_recursive_v2_0_0'],
...
function makeState(templateKey = 'building') {
  ...
  const projectId = `GXS-${APP.releaseDate.replaceAll('-','')}-${Math.floor(100000 + Math.random()*900000)}`;
  return {
    meta:{projectId,lang:'en',createdAt:nowISO(),updatedAt:nowISO(),currentSection:'welcome',institutionGeneration:0,release:APP.version,releaseLineage:{navigatorSeize:'v3.5.0-SN5-BI2-VS2',gymSuccessor:'v3.0.0-GSX3',missionSovereignPaper:'v6.0.0',gymSpecialistASISuccessorOmega:'v6.0.0-unified'},access:null,bridge:null},
    ...
    gym:{version:'ENV-GSSO-1.0.0',formationSeed:20260810,proofSeed:99172026,transferSeed:77192026,formationCases:180,proofCases:260,transferCases:80,distribution:{normal:60,edge:25,adversarial:15},spec:deepClone(template.spec),customScenarios:[]},
    candidates,
...
function loadState() {
  try {
    let stored = localStorage.getItem(APP.storageKey);
    if (!stored) for (const legacyKey of APP.legacyStorageKeys || []) { stored = localStorage.getItem(legacyKey); if (stored) break; }
...
/* ---------- Ω + SEIZE institutional bridge ---------- */
const GSX3_BRIDGE_KEY='goalos.gsx3.bridge.mission';
const GSX3_RESULT_KEY='goalos.gsx3.bridge.result';
function accessSnapshot(){const r=window.GOALOS_GYM_ACCESS?.receipt;return r?{accessClass:r.accessClass,address:r.address,name:r.name||null,observedBalance:r.observedBalance||null,observedBalanceFormatted:r.observedBalanceFormatted||null,lastVerifiedAt:r.lastVerifiedAt||null,expiresAt:r.expiresAt||null,authorityCreated:'NONE'}:null}
function importOmegaMission(payload=null){try{const p=payload||JSON.parse(localStorage.getItem(GSX3_BRIDGE_KEY)||'null');if(!p?.mission)throw Error('No Ω + SEIZE mission bridge is available. Open the Gym from the unified institution and synchronize the mission first.');const m=p.mission;state.mission.name=m.name||m.objective?.slice(0,80)||state.mission.name;state.mission.beneficiary=m.beneficiary||m.owner||state.mission.beneficiary;state.mission.objective=m.objective||state.mission.objective;state.mission.incumbent=m.incumbent||state.mission.incumbent;state.mission.alternatives=Array.isArray(m.alternatives)&&m.alternatives.length?m.alternatives:state.mission.alternatives;state.mission.scope=m.scope||state.mission.scope;state.mission.criticalFailures=Array.isArray(m.criticalFailures)&&m.criticalFailures.length?m.criticalFailures:state.mission.criticalFailures;state.mission.constraints=Array.isArray(m.constraints)&&m.constraints.length?m.constraints:state.mission.constraints;state.mission.authorityCeiling=m.authorityCeiling||state.mission.authorityCeiling;state.mission.proofBudget=Number(m.proofBudget||state.mission.proofBudget);state.meta.bridge={schema:p.schema||'goalos-gsso-bridge/2.0',sourceRelease:p.sourceRelease||null,importedAt:nowISO(),sourceMissionId:p.sourceMissionId||null};state.meta.access=accessSnapshot();state.mission.constitutionFrozen=false;state.proof.result=null;state.proof.frozenCandidateId=null;record('OMEGA_SEIZE_MISSION_IMPORTED','Mission imported from GoalOS Ω + SEIZE',state.mission.name,{bridge:state.meta.bridge});saveState();renderAll();toast('Ω + SEIZE mission imported');return true}catch(error){toast(error.message);return false}}
function buildOmegaResult(){const proof=state.proof.result,candidate=candidateById(state.proof.frozenCandidateId);return{schema:'goalos-gsso-result/2.0',release:APP.version,generatedAt:nowISO(),projectId:state.meta.projectId,mission:{name:state.mission.name,objective:state.mission.objective,incumbent:state.mission.incumbent},candidate:candidate?{id:candidate.id,name:candidate.name,generation:candidate.generation,manifestHash:state.proof.manifestHash}:null,formation:{generation:state.recursive.generation,championId:state.formation.championId,archiveSize:state.recursive.archive.length,negativeCapabilityRecords:state.negativeCapability.length},proof:proof?{pass:proof.pass,status:proof.status,paired:proof.paired,proofCoverage:proof.proofCoverage,criticalErrors:proof.criticalErrors,unauthorizedActions:proof.unauthorizedActions,dimensions:proof.dimensions}:null,authority:{level:state.authority.level,admissionStatus:state.authority.admission.status,expiry:state.authority.expiry,authorityCreated:state.authority.admission.status==='simulated_admitted'?'DEMONSTRATION_ONLY':'NONE'},institutionGeneration:state.meta.institutionGeneration,access:accessSnapshot(),gssOmega:window.GoalOSGSSO?.snapshot?.()||null,claimBoundary:'Deterministic browser-local demonstration. No real-world Specialist ASI, Mission Alpha, professional authority or production release is established.'}}
function publishOmegaResult(){const result=buildOmegaResult();localStorage.setItem(GSX3_RESULT_KEY,JSON.stringify(result));state.meta.access=accessSnapshot();record('OMEGA_SEIZE_RESULT_PUBLISHED','Gym result returned to GoalOS Ω + SEIZE',result.proof?.status||'No fresh proof');if(window.parent!==window)window.parent.postMessage({type:'GOALOS_GYM_RESULT',result},location.origin==='null'?'*':location.origin);saveState();toast('Result returned to Ω + SEIZE');return result}
window.addEventListener('message',event=>{const data=event.data;if(data?.type==='GOALOS_GYM_MISSION'&&data.payload)importOmegaMission(data.payload);if(data?.type==='GOALOS_GYM_ACCESS_RESPONSE'&&data.verified){state.meta.access=accessSnapshot()||{accessClass:data.receipt?.accessClass,address:data.receipt?.address,expiresAt:data.receipt?.expiresAt};saveState()}});
document.addEventListener('goalos:gym-access',event=>{state.meta.access=event.detail?.verified?accessSnapshot():null;saveState()});
...
function renderWelcome() {
  ...
  element.innerHTML = `
    <div class="hero">
      <div class="eyebrow">GoalOS Singularity Navigator Ω + SEIZE · Mission-Sovereign Successors in Singularity Time</div>
      <h1>Unified Verified Succession Institution · Sovereign Mission Gym × Specialist ASI × Successor Ω</h1>
      ...
      <div class="hero-actions">
        ...
        <a class="button secondary" href="../research/current/GoalOS_Unified_Verified_Succession_Institution_v6_0_0_US_LETTER.pdf" target="_blank" rel="noopener">Read Mission-Sovereign Successors v6</a>
        <a class="button secondary" href="../research/current/GoalOS_Unified_Verified_Succession_Institution_v6_0_0_WEB.pdf" target="_blank" rel="noopener">Read Gym · Specialist ASI · Successor Ω</a>
        <button class="button secondary" data-action="import-omega-mission">Import Ω + SEIZE mission</button>
        <button class="button secondary" data-action="publish-omega-result">Return result to Ω + SEIZE</button>
      </div>
    ...
    <div class="alpha-market-band">
      <div class="alpha-market-node"><b>AI BETA</b><strong>Rented</strong><span>The moving market baseline available to competent adopters.</span></div>
      <div class="alpha-market-node"><b>SPREAD</b><strong>${state.seize.score??'—'}</strong><span>The underwritten distance between the incumbent, best external alternative and reachable successor.</span></div>
      <div class="alpha-market-node"><b>SEIZE</b><strong>${esc(state.seize.decision)}</strong><span>Proof-capital allocation under uncertainty, reversibility and tail-risk constraints.</span></div>
      <div class="alpha-market-node alpha"><b>ALPHA</b><strong>${proof?round(proof.paired.lcb,2):'Unproven'}</strong><span>Risk-adjusted mission surplus after comparative fresh proof.</span></div>
      <div class="alpha-market-node"><b>SUCCESSOR</b><strong>${state.authority.admission.status==='simulated_admitted'?'Demo admitted':'Candidate'}</strong><span>A complete institution of models, data, tools, verifier, workflow, evidence, memory and bounded authority.</span></div>
    </div>
    <div class="sovereignty-mini"><span>Mission</span><span>Architecture</span><span>Data</span><span>Evidence</span><span>Memory</span><span>Authority</span><span>Portability</span><span>Economics</span></div>
    ...
    <div class="grid-3">
      ...
      <div class="card"><h2>Exact present status</h2><p>This application is the v3.0.0 integrated, deterministic, browser-local reference implementation and production-quality demonstration of the complete GoalOS succession institution. Its simulation can establish only a <strong>simulation gate result</strong>. Real Specialist ASI and Mission-Sovereign status require protected production-representative evidence, independent review and accountable admission.</p>${statusPill(statusText(),statusTone())}</div>
    </div>
...
function missionPackFiles() {
  ...
  return [
    ...
    {name:`${name}_Authority_Envelope.json`,data:JSON.stringify({schema:'GoalOS.AuthorityEnvelope.v4',...state.authority},null,2)},
    {name:`${name}_Chronicle.json`,data:JSON.stringify({schema:'GoalOS.Chronicle.v4',records:state.chronicle,negativeCapability:state.negativeCapability,successorBook:state.successorBook,requalification:state.requalification},null,2)},
    {name:`${name}_Recursive_Improvement.json`,data:JSON.stringify(recursiveReport(),null,2)},
    {name:`${name}_Bounded_AGI_Jobs.json`,data:JSON.stringify({schema:'GoalOS.BoundedAGIJobPortfolio.v4',jobs:state.jobs},null,2)},
    {name:`${name}_Formation_Results.csv`,data:formationCSV()},
    {name:`${name}_Access_Qualification.json`,data:JSON.stringify(accessSnapshot(),null,2)},
    {name:`${name}_Omega_SEIZE_Bridge.json`,data:JSON.stringify({mission:state.meta.bridge,result:buildOmegaResult()},null,2)},
    {name:`${name}_Release_Constitution_v4_0_0.json`,data:JSON.stringify({release:APP.version,releaseId:'v5.0.0-UVSI1',product:'GoalOS Singularity Navigator Ω + SEIZE with Unified Verified Succession Institution · Sovereign Mission Gym × Specialist ASI × Successor Ω',doctrine:'Beta is rented. The Spread is underwritten. Alpha is manufactured. Specialist ASI is proven. Authority never exceeds fresh proof.',categoricalInvariant:'Mission Gym ≠ Specialist ASI ≠ Successor Ω',access:'Current AGI Club direct owner OR current direct holder of at least 1,000,000 official AGIALPHA on Ethereum Mainnet',qualifyingContract:'0xA61a3B3a130a9c20768EEBF97E21515A6046a1fA',productionReleaseEnabled:false},null,2)}
  ].concat(window.GoalOSGSSO?.missionPackFiles?.(name)||[]);
}
function exportMissionPack() { const name=safeName(state.mission.name); downloadBlob(`${name}_GoalOS_Mission_Pack_v4.zip`,makeZip(missionPackFiles())); record('MISSION_PACK_EXPORTED','Complete Mission Pack exported',name); toast('Mission Pack ZIP downloaded'); }
...
function renderExport() {
  const items=[
    ...
    ['⇧','Import project','Continue from a Version 2, Version 3 or Version 4 project JSON.','import-project'],
...
function tour() {
  modal('Three-minute guided tour',`<ol><li><strong>Triad:</strong> preserve the categorical invariant Mission Gym ≠ Specialist ASI ≠ Successor Ω.</li><li><strong>Mission Physics:</strong> measure observability, controllability, viability, stability and Gym basis risk.</li><li><strong>Nested Gyms:</strong> separate mission, institutional and succession competence.</li><li><strong>Gym Families:</strong> separate formation, attack, transfer, protected proof and canary custody.</li><li><strong>Mission:</strong> freeze a narrow objective, incumbent, alternatives, failures, proof budget and authority ceiling.</li><li><strong>Manifold:</strong> compare complete successor institutions, not model brands.</li><li><strong>Gradient:</strong> estimate the bounded architecture change with the greatest marginal Alpha per unit of proof capital.</li><li><strong>Alpha/Beta/Spread:</strong> charge Mission Beta, complete burden and basis risk before calling the residual Alpha.</li><li><strong>SEIZE:</strong> underwrite whether to retain, repair, rent, build, partner, acquire, hedge, reserve or stop.</li><li><strong>Bounded AGI Jobs:</strong> compile the 21 work-and-proof contracts.</li><li><strong>Formation Gym:</strong> run matched cases and preserve a quality-diversity archive.</li><li><strong>Recursive Foundry:</strong> produce descendants, reject regressions and retain failures.</li><li><strong>Fresh Proof:</strong> freeze one challenger and purchase protected comparative evidence.</li><li><strong>Five Decisions:</strong> keep Underwrite, Execute, Accept, Admit and Allocate separately signed.</li><li><strong>Authority:</strong> rehearse refusal, escalation, degraded mode, expiry and rollback.</li><li><strong>Successor Ω:</strong> require accountable admission, handover, a sovereign Gym ledger and a non-bypassable recursion firewall.</li><li><strong>Chronicle:</strong> admit only rights-cleared evidence and use it to improve the next succession cycle.</li><li><strong>Requalification:</strong> treat every material change as a new release.</li><li><strong>Export:</strong> download the complete Mission Pack.</li></ol><div class="callout red"><strong>Remember:</strong> simulation performance is candidacy—not real-world Specialist ASI, Mission Alpha or institutional authority.</div>`);
}
async function guidedRun() {
  const hasBridge=Boolean(state.meta?.bridge&&state.mission?.objective);
  const question=hasBridge?'Run the complete local guided demonstration on the synchronized Ω + SEIZE mission? Derived candidates, evidence and admission state will be rebuilt; the synchronized Mission Constitution will be preserved.':'Run the complete local guided demonstration? It will reset the current project to the Portfolio Building Operations mission.';
  if (!confirm(question)) return;
  const preservedMission=hasBridge?deepClone(state.mission):null, preservedBridge=hasBridge?deepClone(state.meta.bridge):null, preservedAccess=accessSnapshot();
  state=makeState(hasBridge?(preservedMission.template||'custom'):'building');
  if(hasBridge){state.mission={...state.mission,...preservedMission,constitutionFrozen:false,frozenAt:null};state.meta.bridge=preservedBridge;state.meta.access=preservedAccess;}
  saveState(); renderAll();
...
function handleAction(action,target) {
  if(action==='tour')tour();
  if(action==='import-omega-mission')importOmegaMission();
  if(action==='publish-omega-result')publishOmegaResult();
...
function init() {
  ...
  if(!state.chronicle.length)record('PROJECT_CREATED','GoalOS Gym × Successor project created',`${state.mission.name} · ${APP.edition}.`);
  renderAll();
  if(window.parent!==window)window.parent.postMessage({type:'GOALOS_GYM_READY',release:APP.version},location.origin==='null'?'*':location.origin);
}
...
window.GoalOSDemo = {
  ...
  buildOmegaResult,
  guidedRun,
  issueSimulatedAdmission,
  maximumEligibleAuthority
};
