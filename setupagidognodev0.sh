...
# AGI Dog Node v0 - setupagidognodev0.sh
...
# Step 2: Stop and remove any existing container named "ollama_dog_v0"
docker stop ollama_dog_v0 2>/dev/null || true
docker rm ollama_dog_v0 2>/dev/null || true
...
docker run -d -v ollama_dog_v0:/root/.ollama -p 11440:11434 --name ollama_dog_v0 ollama/ollama || exit 1

# Step 5: Create the Modelfile inside the Docker container
docker exec -i ollama_dog_v0 /bin/sh << 'EOF'
...
His name on this date is dynamically set based on the successfully verified AGI Dog Subdomain Identity with the first letter capitalized.
...
docker exec -it ollama_dog_v0 ollama create agidog_v0 -f /root/Modelfile
...
def chat():
    ...
    result = subprocess.run(
        ['docker', 'exec', '-i', 'ollama_dog_v0', 'ollama', 'run', 'agidog_v0'],
...
# Create index_person1.html for Person 1 (AGI Dog User)
cat << 'EOF' > agidog_app/templates/index_person1.html
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="container">
        <h1 class="title">AGI Dog v0 🤍✨</h1>
        <img src="/static/agidog.png" alt="AGI Dog" class="agidog-image">
        <div class="section">
            <label for="subdomain">Enter Your AGI Dog Subdomain Identity:</label>
            <input type="text" id="subdomain" name="subdomain">
            <button id="verifySubdomainBtn">Verify AGI Dog ENS Identity Ownership</button>
            <p id="output"></p>
        </div>
        <div class="section">
            <label for="tokenId">Enter Your AGI Dog NFT Token ID:</label>
            <input type="text" id="tokenId" name="tokenId">
            <button id="verifyNFTBtn">Verify AGI Dog NFT Ownership</button>
            <p id="nftOutput"></p>
        </div>
        <div class="chat-container" id="chat-container" style="display: none;">
            <div class="chat-header">
