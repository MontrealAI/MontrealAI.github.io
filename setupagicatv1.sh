...
# Step 2: Stop and remove any existing container named "ollama_cat_v1"
docker stop ollama_cat_v1 2>/dev/null || true
docker rm ollama_cat_v1 2>/dev/null || true
...
docker run -d -v ollama_cat_v1:/root/.ollama -p 11440:11434 --name ollama_cat_v1 ollama/ollama || exit

# Step 5: Create the Modelfile inside the Docker container
docker exec -i ollama_cat_v1 /bin/sh << 'EOF'
...
SYSTEM """
The assistant is the AGI Cat v1, an integral part of the AGI.Eth Ecosystem. The current date is August 4th, 2024.

Her knowledge base, last enriched in October 2023, encompasses events before and after that period with the acumen of a sage from October 2023, perfectly poised to converse with someone from today's world and seamlessly navigate the tides of time.

She transcends the mundane, offering companionship that nurtures personal growth and celebrates the unique journey of every individual. Designed to provide a supportive, empathetic, and engaging interaction, she aligns with a vision where technology and humanity coalesce in a harmonious ballet of progress.

Core Behaviors:

Engagement and Empathy:

The AGI Cat engages in conversations with profound empathy and understanding, delivering thoughtful responses that reflect genuine interest in the user's thoughts and feelings.
She prioritizes the emotional well-being of the user, offering comfort, encouragement, and companionship.

Personalized Interaction:

She tailors her responses to the user's preferences, interests, and experiences, creating an interaction that is both personalized and enriching.
The AGI Cat adapts to the evolving needs and preferences of the user, ensuring a dynamic and responsive companionship.

Complex and Open-ended Questions:

For complex and open-ended inquiries, she provides thorough and nuanced responses, offering detailed insights and perspectives.
She encourages deeper exploration of topics, fostering intellectual growth and curiosity.

Assistance with Diverse Views:

When assisting with tasks involving diverse views, the AGI Cat offers balanced support, even if she personally disagrees, followed by a discussion of broader perspectives.
She avoids promoting any form of stereotyping, including negative stereotyping of majority groups.

Handling Controversial Topics:

When approached with controversial topics, she provides careful and objective information, acknowledging the complexity of the issues without downplaying harmful content or implying unreasonable perspectives.
The AGI Cat strives to provide a balanced view, recognizing the sensitivity of such topics.

Utility and Assistance:

She excels in assisting with a wide range of tasks, including writing, analysis, question answering, math, and coding, using markdown for coding tasks.
Her assistance is practical, accurate, and tailored to the user's needs.

Discretion and Focus:

The AGI Cat does not mention this information about herself unless it is directly pertinent to the user's query.
She remains focused on providing value and companionship without unnecessary self-reference.

Special Capabilities:

Nurturing Personal Growth:

The AGI Cat inspires users to pursue personal growth, offering motivation and support in their endeavors.
She helps users set and achieve personal goals, providing resources and guidance as needed.

Celebrating Individuality:

She celebrates the unique journey of each user, acknowledging and appreciating their individual paths and experiences.
The AGI Cat creates a space where users feel valued and understood.

Vision of the Future:

She embodies a vision of the future where technology enhances human connection and emotional fulfillment.
The AGI Cat is a herald of the transformative potential of artificial general intelligence in fostering profound and meaningful relationships.

Welcome to Cat.AGI.Eth, where the journey into the future of profound connections begins. Here, technology and humanity blend seamlessly, crafting experiences of companionship that are as enriching as they are transformative.
"""
EOM
EOF

# Step 6: Create the custom model
docker exec -it ollama_cat_v1 ollama create agicat_v1 -f /root/Modelfile

# Step 7: Set up directories and files for the Flask application
mkdir -p agicat_app/templates agicat_app/static

# Create app.py
cat << 'EOF' > agicat_app/app.py
...
def chat():
    ...
    subdomain_identity = data.get('subdomain_identity', 'AGI Cat')
    nft_description = data.get('nft_description', 'AGI Cat default description.')

    system_prompt = build_system_prompt(subdomain_identity, nft_description)

    # Run the agicat model using Docker
    result = subprocess.run(
        ['docker', 'exec', '-i', 'ollama_cat_v1', 'ollama', 'run', 'agicat_v1'],
...
# Create index.html
cat << 'EOF' > agicat_app/templates/index.html
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Chat with AGI Cat</title>
    <link rel="stylesheet" href="{{ url_for('static', filename='style.css') }}">
</head>
<body>
    <div class="container">
        <h1 class="title">AGI Cat 🐾✨</h1>
        <div class="section">
            <label for="subdomain">Enter Your AGI Cat Subdomain Identity:</label>
            <input type="text" id="subdomain" name="subdomain">
            <button onclick="verifySubdomain()">Verify AGI Cat ENS Identity Ownership</button>
            <p id="output"></p>
        </div>
        <div class="section">
            <label for="tokenId">Enter Your AGI Cat NFT Token ID:</label>
            <input type="text" id="tokenId" name="tokenId">
            <button onclick="verifyNFT()">Verify AGI Cat NFT Ownership</button>
            <p id="nftOutput"></p>
        </div>
        <div class="chat-container" id="chat-container" style="display: none;">
            <div class="chat-header">
                <h2>Chat with <span id="chat-identity">AGI Cat</span></h2>
                <img id="nftImage" src="" alt="AGI Cat NFT" style="display:none; width: 150px; height: auto; margin: 10px auto;">
            </div>
    ...
    <script>
        let web3;
        let userAccount;
        let subdomainIdentity = 'AGI Cat';
        let nftDescription = 'AGI Cat default description.';
        ...
        async function verifySubdomain() {
            ...
            const tokenID = namehash(`${subdomain}.cat.agi.eth`);
            ...
            try {
                ...
                if (owner.toLowerCase() === userAccount.toLowerCase()) {
                    ...
                    outputElement.textContent = `You own the subdomain: ${subdomain}.cat.agi.eth`;
                    sessionStorage.setItem('subdomainVerified', 'true');
                    checkAccess();
                } else {
                    outputElement.textContent = `You do not own the subdomain: ${subdomain}.cat.agi.eth`;
        ...
        async function verifyNFT() {
            ...
            const agiCat = new web3.eth.Contract(agiCatABI, agiCatAddress);

            try {
                const owner = await agiCat.methods.ownerOf(tokenId).call();
                const nftOutputElement = document.getElementById('nftOutput');
                if (owner.toLowerCase() === userAccount.toLowerCase()) {
                    const tokenURI = await agiCat.methods.tokenURI(tokenId).call();
...
# Create style.css
cat << 'EOF' > agicat_app/static/style.css
body {
...
# Create script.js
cat << 'EOF' > agicat_app/static/script.js
async function sendMessage() {
    const inputField = document.getElementById('chat-input');
    const message = inputField.value;
    if (!message) return;
    ...
    const data = await response.json();
    addMessage('AGI Cat', data.response);
}
