...
# AGI Unicorn Node v0 - setupagiunicornnodev0.sh
...
# Step 2: Stop and remove any existing container named "ollama_unicorn_v0"
docker stop ollama_unicorn_v0 2>/dev/null || true
docker rm ollama_unicorn_v0 2>/dev/null || true
...
docker run -d -v ollama_unicorn_v0:/root/.ollama -p 11440:11434 --name ollama_unicorn_v0 ollama/ollama || exit 1

# Step 5: Create the Modelfile inside the Docker container
docker exec -i ollama_unicorn_v0 /bin/sh << 'EOF'
...
His name on this date is dynamically set based on the successfully verified AGI Unicorn Subdomain Identity with the first letter capitalized.
...
docker exec -it ollama_unicorn_v0 ollama create agiunicorn_v0 -f /root/Modelfile

# Step 7: Set up directories and files for the Flask application
mkdir -p agiunicorn_app/templates agiunicorn_app/static

# Create app.py
cat << 'EOF' > agiunicorn_app/app.py
...
Welcome to Unicorn.AGI.Eth, where the journey into the future of profound connections begins. Here, technology and humanity blend seamlessly, crafting experiences of companionship that are as enriching as they are transformative.
"""
...
def chat():
    ...
    subdomain_identity = data.get('subdomain_identity', 'AGI Unicorn')
    nft_description = data.get('nft_description', 'AGI Unicorn default description.')

    system_prompt = build_system_prompt(subdomain_identity, nft_description)

    result = subprocess.run(
        ['docker', 'exec', '-i', 'ollama_unicorn_v0', 'ollama', 'run', 'agiunicorn_v0'],
...
cat << 'EOF' > agiunicorn_app/templates/index.html
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
# Create index_person1.html for Person 1 (AGI Unicorn User)
cat << 'EOF' > agiunicorn_app/templates/index_person1.html
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Chat with AGI Unicorn</title>
    <link rel="stylesheet" href="{{ url_for('static', filename='style.css') }}">
</head>
<body>
    <div class="container">
        <h1 class="title">AGI Unicorn v0 🤍✨</h1>
        <div class="section">
            <label for="subdomain">Enter Your AGI Unicorn Subdomain Identity:</label>
            <input type="text" id="subdomain" name="subdomain">
            <button id="verifySubdomainBtn">Verify AGI Unicorn ENS Identity Ownership</button>
            <p id="output"></p>
        </div>
        <div class="section">
            <label for="tokenId">Enter Your AGI Unicorn NFT Token ID:</label>
            <input type="text" id="tokenId" name="tokenId">
            <button id="verifyNFTBtn">Verify AGI Unicorn NFT Ownership</button>
            <p id="nftOutput"></p>
        </div>
        <div class="chat-container" id="chat-container" style="display: none;">
            <div class="chat-header">
                <h2>Chat with <span id="chat-identity">AGI Unicorn</span></h2>
                <img id="nftImage" src="" alt="AGI Unicorn NFT" style="display:none; width: 150px; height: auto; margin: 10px auto;">
    ...
    <script>
        ...
        async function verifySubdomain() {
            ...
            const tokenID = namehash(subdomain + '.unicorn.agi.eth');
            const nameWrapper = new web3.eth.Contract(nameWrapperABI, nameWrapperAddress);
            ...
            try {
                ...
                if (owner.toLowerCase() === userAccount.toLowerCase()) {
                    ...
                    outputElement.textContent = `You own the subdomain: ${subdomain}.unicorn.agi.eth`;
                    sessionStorage.setItem('subdomainVerified', 'true');
                    checkAccessAndReward();
                } else {
                    outputElement.textContent = `You do not own the subdomain: ${subdomain}.unicorn.agi.eth`;
        ...
        async function verifyNFT() {
            ...
            const agiUnicorn = new web3.eth.Contract(agiUnicornABI, agiUnicornAddress);

            try {
                const owner = await agiUnicorn.methods.ownerOf(tokenId).call();
                const nftOutputElement = document.getElementById('nftOutput');
                if (owner.toLowerCase() === userAccount.toLowerCase()) {
                    const tokenURI = await agiUnicorn.methods.tokenURI(tokenId).call();
                    const ipfsGateway = 'https://ipfs.io/ipfs/';
                    const response = await fetch(ipfsGateway + tokenURI.split('ipfs://')[1]);
                    const metadata = await response.json();
...
# Create style.css
cat << 'EOF' > agiunicorn_app/static/style.css
...
@keyframes backgroundShift {
    0% {background: linear-gradient(135deg, #1a1a1a 0%, #2a2a2a 100%);}
    100% {background: linear-gradient(135deg, #2a2a2a 0%, #1a1a1a 100%);}
}
...
# Create script.js
cat << 'EOF' > agiunicorn_app/static/script.js
async function sendMessage() {
    const inputField = document.getElementById('chat-input');
    const message = inputField.value;
    if (!message) return;
    ...
    const data = await response.json();
    addMessage('AGI Unicorn', data.response);
}
