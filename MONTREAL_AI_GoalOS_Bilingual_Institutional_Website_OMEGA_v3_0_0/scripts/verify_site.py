...
PUBLIC_EXCLUDE={Path('START_HERE.html')}
PUBLIC_EXCLUDE_PREFIXES=('goalos-documents/','BUILD_SOURCE/','qa/')
...
all_html=sorted(p for p in ROOT.rglob('*.html') if p.relative_to(ROOT) not in PUBLIC_EXCLUDE and not p.relative_to(ROOT).as_posix().startswith(PUBLIC_EXCLUDE_PREFIXES) and '_Downloads' not in p.as_posix())
for f in all_html:
...
legal={'index.html','terms/index.html','privacy/index.html','claim-boundary/index.html','regulatory/index.html','acceptable-use/index.html','web3-risk/index.html','ai-transparency/index.html','security/index.html','accessibility/index.html','enterprise-services/index.html','non-confidential/index.html'}
