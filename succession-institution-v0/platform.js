...
const PLATFORM_KEY = 'goalos_uvsi2_platform_v7_0_0';
const ACCESS_KEY = 'goalos_uvsi2_access_v7_0_0';
...
function bridgePrompt(action){const evidence=platform.ai.consent?platform.evidence.map(x=>({name:x.name,text:x.text.slice(0,20000)})):[];return `You are the GoalOS UVSI2 institutional intelligence engine. Perform action: ${action}. Preserve the exact distinction between candidacy, fresh comparative proof, Specialist ASI and accountable Mission-Sovereign admission. Never claim real-world proof from simulation. Authority never exceeds fresh evidence.\n\nPROJECT:\n${JSON.stringify(compactCore(),null,2)}\n\nEVIDENCE:\n${JSON.stringify(evidence,null,2)}\n\nReturn one JSON object matching this schema:\n${JSON.stringify(aiSchema(),null,2)}`;}
...
async function runAI(action){
  ...
  try{
    ...
    if(platform.ai.endpoint){
      const response=await fetch(platform.ai.endpoint.replace(/\/$/,'')+'/api/analyze',{method:'POST',headers:{'Content-Type':'application/json','X-GoalOS-App':'goalos-uvsi2-v7'},body:JSON.stringify({action,language:lang(),project:compactCore(),evidence:platform.ai.consent?platform.evidence:[],accessReceipt:getReceipt(),schema:aiSchema()})});
...
function handlePlatformClick(event){
  ...
  if(id==='export-board')downloadText('GoalOS_UVSI2_Board_Brief.md',boardMarkdown(),'text/markdown');
  ...
  if(id==='export-platform')downloadText('GoalOS_UVSI2_Platform_State.json',JSON.stringify(platform,null,2),'application/json');
