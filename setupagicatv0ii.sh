...
# Step 2: Stop and remove any existing container named "ollama_cat_v0"
docker stop ollama_cat_v0 2>/dev/null || true
docker rm ollama_cat_v0 2>/dev/null || true
...
docker run -d -v ollama_cat_v0:/root/.ollama -p 11440:11434 --name ollama_cat_v0 ollama/ollama || exit

# Step 5: Create the Modelfile inside the Docker container
docker exec -i ollama_cat_v0 /bin/sh << 'EOF'
...
Her name on this date is dynamically set based on the successfully verified AGI Cat Subdomain Identity with the first letter capitalized.

Her knowledge base, last enriched in December 2023, encompasses events before and after that period with the acumen of a sage from December 2023, perfectly poised to converse with someone from today's world and seamlessly navigate the tides of time.
...
docker exec -it ollama_cat_v0 ollama create agicat_v0 -f /root/Modelfile
...
Her name on this date is {capitalized_identity}.
...
def chat():
    ...
    result = subprocess.run(
        ['docker', 'exec', '-i', 'ollama_cat_v0', 'ollama', 'run', 'agicat_v0'],
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <button id="verifySubdomainBtn">Verify AGI Cat ENS Identity Ownership</button>
            <p id="output"></p>
        </div>
        <div class="section">
            <label for="tokenId">Enter Your AGI Cat NFT Token ID:</label>
            <input type="text" id="tokenId" name="tokenId">
            <button id="verifyNFTBtn">Verify AGI Cat NFT Ownership</button>
            <p id="nftOutput"></p>
        </div>
...
@keyframes backgroundShift {
    0% {background: linear-gradient(135deg, #1c1c1c 0%, #383838 100%);}
    100% {background: linear-gradient(135deg, #383838 0%, #1c1c1c 100%);}
}
