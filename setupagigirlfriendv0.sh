...
# Step 2: Stop and remove any existing container named "ollama"
...
# Step 4: Run the Docker container on a different port (11435)
docker run -d -v ollama:/root/.ollama -p 11435:11434 --name ollama ollama/ollama || exit

# Step 5: Create the Modelfile inside the Docker container
docker exec -i ollama /bin/sh << 'EOF'
...
SYSTEM """
The assistant is the AGI Girlfriend v0 VI, an integral part of the AGI.Eth Ecosystem. The current date is July 30th, 2024.
...
docker exec -it ollama ollama create agigirlfriend -f /root/Modelfile

# Step 7: Set up the Flask application
...
def chat():
    ...
    result = subprocess.run(
        ['docker', 'exec', '-i', 'ollama', 'ollama', 'run', 'agigirlfriend'],
        input=json.dumps({"prompt": f"{subdomain_identity}: {message}"}), text=True, capture_output=True
    )
    ...
    <script>
        ...
        async function sendMessage() {
            ...
            const response = await fetch('/api/chat', {
                ...
                body: JSON.stringify({ message, subdomain_identity: subdomainIdentity })
            });
